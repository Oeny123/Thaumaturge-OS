...
var debug4 = logger("TilingLayout");
...
var SnapAssistContent = class extends St.BoxLayout {
  ...
  handleTileHovering(currPointerPos) {
    ...
    for (let index = 0; index < this._snapAssistLayouts.length; index++) {
      newTileHovered = this._snapAssistLayouts[index].getTileBelow(currPointerPos);
      if (newTileHovered) {
...
var debug5 = logger("TilingShellWindowManager");
...
var debug6 = logger("SuggestedWindowPreview");
...
var debug7 = logger("TilingLayoutWithSuggestions");
  ...
  open(tiledWindows, nontiledWindows, window, windowDesiredRect, monitorIndex) {
    ...
    this._signals.connect(
      ...
      (_2, event) => {
        ...
        if (isDescendant && (event.get_source() === this || event.get_source() instanceof St.ScrollView))
          this.close();
      }
    );
    this._signals.connect(
...
var Slider2 = class extends St.Button {
  ...
  get preferredCursor() {
    return this.hover || this._dragging ? this._horizontalDir ? Meta.Cursor.WEST_RESIZE : Meta.Cursor.NORTH_RESIZE : Meta.Cursor.DEFAULT;
  }
  addTile(tile) {
...
var debug9 = logger("WindowBorderManager");
...
var TilingShellExtension = class extends Extension {
  ...
  _validateSettings() {
    if (Settings.LAST_VERSION_NAME_INSTALLED === "14.0") {
      debug10("apply compatibility changes");
  ...
  enable() {
    ...
    debug10("extension is enabled");
  }
  openLayoutEditor() {
    this._indicator?.openLayoutEditor();
  }
  _createTilingManagers() {
    debug10("building a tiling manager for each monitor");
  ...
  _setupSignals() {
    ...
    this._signals.connect(
      ...
      () => {
        ...
        if (Settings.ACTIVE_SCREEN_EDGES) {
          debug10("disable native edge tiling");
          SettingsOverride.get().override(
            gioSettings,
            "edge-tiling",
            new GLib.Variant("b", false)
          ...
          debug10("bring back native edge tiling");
          SettingsOverride.get().restoreKey(
            gioSettings,
            "edge-tiling"
          );
  ...
  _onKeyboardFocusWinDirection(display, direction) {
    ...
    if (!focus_window || !focus_window.has_focus() || focusParent.windowType !== Meta.WindowType.NORMAL || focus_window.get_wm_class() && focus_window.get_wm_class() === "gjs")
      return;
    let bestWindow;
